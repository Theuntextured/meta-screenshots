...
public class Screenshot {
    ...
    public Screenshot(ScreenshotEvent screenshotEvent){
        ...
        CompletableFuture.runAsync(() -> ImageHelper.writeImageWithMetadata(rawPngBytes, targetFile, metadataPayload));
    }
    ...
    private ResourceLocation loadTextureIntoVRAM(File file, String suffix) {
        try {
            ...
            return Minecraft.getInstance().getTextureManager().register(
                    "meta_screenshots/" + file.getName().toLowerCase().replace(".png", "") + suffix,
                    texture
            );
        } catch (Exception e) {
